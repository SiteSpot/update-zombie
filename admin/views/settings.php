...
$update_zombie_name       = Update_Zombie_Settings::OPTION;
$update_zombie_schedules  = wp_get_schedules();
$update_zombie_key_source = Update_Zombie_Credentials::key_source();
?>
<div class="wrap update-zombie-wrap">
	...
	<?php settings_errors( $update_zombie_name ); ?>
	...
	<form method="post" action="options.php">
				...
				<td>
					<fieldset>
						<label class="uz-radio">
							<input type="radio" name="<?php echo esc_attr( $update_zombie_name ); ?>[analysis_engine]" value="<?php echo esc_attr( Update_Zombie_Settings::ENGINE_AI ); ?>" <?php checked( $settings['analysis_engine'], Update_Zombie_Settings::ENGINE_AI ); ?>>
						...
						<label class="uz-radio">
							<input type="radio" name="<?php echo esc_attr( $update_zombie_name ); ?>[analysis_engine]" value="<?php echo esc_attr( Update_Zombie_Settings::ENGINE_SIGNALS ); ?>" <?php checked( $settings['analysis_engine'], Update_Zombie_Settings::ENGINE_SIGNALS ); ?>>
		...
		<table class="form-table" role="presentation">
			<tr>
				...
				<td>
					<?php if ( Update_Zombie_Credentials::SOURCE_CONSTANT === $update_zombie_key_source ) : ?>
						<p>
					...
					<?php else : ?>
						<input type="password" id="uz-key" name="<?php echo esc_attr( $update_zombie_name ); ?>[openrouter_key]" value="" class="regular-text" autocomplete="off" placeholder="<?php echo esc_attr( Update_Zombie_Credentials::SOURCE_OPTION === $update_zombie_key_source ? Update_Zombie_Credentials::masked_key() : 'sk-or-v1-…' ); ?>">

						<?php if ( Update_Zombie_Credentials::SOURCE_OPTION === $update_zombie_key_source ) : ?>
							...
							<p>
								<label>
									<input type="checkbox" name="<?php echo esc_attr( $update_zombie_name ); ?>[openrouter_key_clear]" value="1">
			...
			<tr>
				...
				<td>
					<input type="text" id="uz-model" name="<?php echo esc_attr( $update_zombie_name ); ?>[model_preference]" value="<?php echo esc_attr( (string) $settings['model_preference'] ); ?>" class="regular-text" list="uz-model-list">
					<datalist id="uz-model-list">
						<?php foreach ( Update_Zombie_OpenRouter_Directory::known_models() as $update_zombie_id => $update_zombie_label ) : ?>
							<option value="<?php echo esc_attr( $update_zombie_id ); ?>"><?php echo esc_html( $update_zombie_label ); ?></option>
						<?php endforeach; ?>
					</datalist>
					<p class="description">
			...
			<tr>
				...
				<td>
					<input type="number" min="30" max="900" id="uz-timeout" name="<?php echo esc_attr( $update_zombie_name ); ?>[request_timeout]" value="<?php echo esc_attr( (string) $settings['request_timeout'] ); ?>" class="small-text">
		...
		<table class="form-table" role="presentation">
			<tr>
				...
				<td>
					<fieldset>
						...
						<label class="uz-radio">
							<input type="radio" name="<?php echo esc_attr( $update_zombie_name ); ?>[mode]" value="<?php echo esc_attr( Update_Zombie_Settings::MODE_ADVISORY ); ?>" <?php checked( $settings['mode'], Update_Zombie_Settings::MODE_ADVISORY ); ?>>
						...
						<label class="uz-radio">
							<input type="radio" name="<?php echo esc_attr( $update_zombie_name ); ?>[mode]" value="<?php echo esc_attr( Update_Zombie_Settings::MODE_GUARDED ); ?>" <?php checked( $settings['mode'], Update_Zombie_Settings::MODE_GUARDED ); ?>>
							<strong><?php esc_html_e( 'Guarded', 'update-zombie' ); ?></strong>
							<span class="description"><?php esc_html_e( 'Install high-impact security fixes automatically. Hold back anything judged questionable or worth avoiding until you approve it. Lower-impact fixes and everything else follow your normal WordPress settings.', 'update-zombie' ); ?></span>
						</label>

						<label class="uz-radio">
							<input type="radio" name="<?php echo esc_attr( $update_zombie_name ); ?>[mode]" value="<?php echo esc_attr( Update_Zombie_Settings::MODE_AUTOPILOT ); ?>" <?php checked( $settings['mode'], Update_Zombie_Settings::MODE_AUTOPILOT ); ?>>
							<strong><?php esc_html_e( 'Autopilot', 'update-zombie' ); ?></strong>
							<span class="description"><?php esc_html_e( 'Install high-impact security fixes and updates judged good. Hold back questionable ones and ones worth avoiding. The most hands-off setting, and the one that trusts the model most.', 'update-zombie' ); ?></span>
						</label>
					</fieldset>
			...
			<tr>
				...
				<td>
					<input type="number" min="0" max="100" id="uz-security-confidence" name="<?php echo esc_attr( $update_zombie_name ); ?>[security_confidence]" value="<?php echo esc_attr( (string) $settings['security_confidence'] ); ?>" class="small-text">
					<span>%</span>
		...
		<table class="form-table" role="presentation">
			<tr>
				...
				<td>
					<fieldset>
						<label><input type="checkbox" name="<?php echo esc_attr( $update_zombie_name ); ?>[watch_plugins]" value="1" <?php checked( $settings['watch_plugins'] ); ?>> <?php esc_html_e( 'Plugin updates', 'update-zombie' ); ?></label><br>
						<label><input type="checkbox" name="<?php echo esc_attr( $update_zombie_name ); ?>[watch_themes]" value="1" <?php checked( $settings['watch_themes'] ); ?>> <?php esc_html_e( 'Theme updates', 'update-zombie' ); ?></label><br>
						<label><input type="checkbox" name="<?php echo esc_attr( $update_zombie_name ); ?>[watch_core]" value="1" <?php checked( $settings['watch_core'] ); ?>> <?php esc_html_e( 'WordPress core updates', 'update-zombie' ); ?></label>
					</fieldset>
			...
			<tr>
				...
				<td>
					<label>
						<input type="checkbox" name="<?php echo esc_attr( $update_zombie_name ); ?>[core_majors]" value="1" <?php checked( $settings['core_majors'] ); ?>>
			...
			<tr>
				...
				<td>
					<select id="uz-interval" name="<?php echo esc_attr( $update_zombie_name ); ?>[analysis_interval]">
						<?php foreach ( $update_zombie_schedules as $update_zombie_key => $update_zombie_schedule ) : ?>
							<option value="<?php echo esc_attr( $update_zombie_key ); ?>" <?php selected( $settings['analysis_interval'], $update_zombie_key ); ?>>
								<?php echo esc_html( $update_zombie_schedule['display'] ); ?>
							</option>
						<?php endforeach; ?>
					</select>
		...
		<table class="form-table" role="presentation">
			<tr>
				...
				<td>
					<select id="uz-budget" name="<?php echo esc_attr( $update_zombie_name ); ?>[diff_char_budget]">
						<?php
						$update_zombie_budgets = Update_Zombie_Settings::budget_presets();
						$update_zombie_current = (int) $settings['diff_char_budget'];

						// Keep a custom value set by a filter or an older version
						// selectable, so opening this screen never silently
						// changes it.
						if ( ! isset( $update_zombie_budgets[ $update_zombie_current ] ) ) {
							$update_zombie_budgets[ $update_zombie_current ] = sprintf(
								/* translators: %s: number of characters. */
								__( '%s characters (custom)', 'update-zombie' ),
								number_format_i18n( $update_zombie_current )
							);
							ksort( $update_zombie_budgets );
						}

						foreach ( $update_zombie_budgets as $update_zombie_value => $update_zombie_label ) :
							?>
							<option value="<?php echo esc_attr( (string) $update_zombie_value ); ?>" <?php selected( $update_zombie_current, $update_zombie_value ); ?>>
								<?php echo esc_html( $update_zombie_label ); ?>
							</option>
						<?php endforeach; ?>
					</select>
			...
			<tr>
				...
				<td>
					<select id="uz-max-file" name="<?php echo esc_attr( $update_zombie_name ); ?>[max_file_bytes]">
						<?php
						$update_zombie_sizes        = Update_Zombie_Settings::file_size_presets();
						$update_zombie_current_size = (int) $settings['max_file_bytes'];

						if ( ! isset( $update_zombie_sizes[ $update_zombie_current_size ] ) ) {
							$update_zombie_sizes[ $update_zombie_current_size ] = sprintf(
								/* translators: %s: formatted file size. */
								__( '%s (custom)', 'update-zombie' ),
								size_format( $update_zombie_current_size )
							);
							ksort( $update_zombie_sizes );
						}

						foreach ( $update_zombie_sizes as $update_zombie_value => $update_zombie_label ) :
							?>
							<option value="<?php echo esc_attr( (string) $update_zombie_value ); ?>" <?php selected( $update_zombie_current_size, $update_zombie_value ); ?>>
								<?php echo esc_html( $update_zombie_label ); ?>
							</option>
						<?php endforeach; ?>
					</select>
			...
			<tr>
				...
				<td>
					<input type="number" min="1" max="3650" id="uz-retention" name="<?php echo esc_attr( $update_zombie_name ); ?>[retention_days]" value="<?php echo esc_attr( (string) $settings['retention_days'] ); ?>" class="small-text">
		...
		<table class="form-table" role="presentation">
			<tr>
				...
				<td>
					<label><input type="checkbox" name="<?php echo esc_attr( $update_zombie_name ); ?>[notify_email]" value="1" <?php checked( $settings['notify_email'] ); ?>> <?php esc_html_e( 'Send email notifications', 'update-zombie' ); ?></label>
					<p>
						<input type="email" name="<?php echo esc_attr( $update_zombie_name ); ?>[notify_email_address]" value="<?php echo esc_attr( (string) $settings['notify_email_address'] ); ?>" class="regular-text" placeholder="<?php echo esc_attr( (string) get_option( 'admin_email' ) ); ?>">
			...
			<tr>
				...
				<td>
					<fieldset>
						<label><input type="checkbox" name="<?php echo esc_attr( $update_zombie_name ); ?>[notify_on_security]" value="1" <?php checked( $settings['notify_on_security'] ); ?>> <?php esc_html_e( 'An update is a security fix, or one is installed automatically', 'update-zombie' ); ?></label><br>
						<label><input type="checkbox" name="<?php echo esc_attr( $update_zombie_name ); ?>[notify_on_concerns]" value="1" <?php checked( $settings['notify_on_concerns'] ); ?>> <?php esc_html_e( 'An update raises concerns', 'update-zombie' ); ?></label><br>
						<label><input type="checkbox" name="<?php echo esc_attr( $update_zombie_name ); ?>[notify_on_held]" value="1" <?php checked( $settings['notify_on_held'] ); ?>> <?php esc_html_e( 'An update is held back', 'update-zombie' ); ?></label><br>
						<label><input type="checkbox" name="<?php echo esc_attr( $update_zombie_name ); ?>[notify_on_error]" value="1" <?php checked( $settings['notify_on_error'] ); ?>> <?php esc_html_e( 'An analysis fails', 'update-zombie' ); ?></label>
					</fieldset>
				</td>
			</tr>
			...
			<tr>
				...
				<td>
					<label><input type="checkbox" name="<?php echo esc_attr( $update_zombie_name ); ?>[webhook_enabled]" value="1" <?php checked( $settings['webhook_enabled'] ); ?>> <?php esc_html_e( 'POST each verdict to a URL', 'update-zombie' ); ?></label>
					<p>
						<input type="url" name="<?php echo esc_attr( $update_zombie_name ); ?>[webhook_url]" value="<?php echo esc_attr( (string) $settings['webhook_url'] ); ?>" class="large-text" placeholder="https://hooks.example.com/…">
					</p>
					<p>
						<input type="text" name="<?php echo esc_attr( $update_zombie_name ); ?>[webhook_secret]" value="<?php echo esc_attr( (string) $settings['webhook_secret'] ); ?>" class="regular-text" placeholder="<?php esc_attr_e( 'Signing secret (optional)', 'update-zombie' ); ?>">
